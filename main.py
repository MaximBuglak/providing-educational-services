...
# ФУНКЦИЯ ЧТЕНИЯ CSV С ДИСКА
def load_local_csv():
    csv_path = "data/education_finance.csv"
    if os.path.exists(csv_path):
        ...
        # Прибыль уже есть, но на всякий случай пересчитаем
...
app.layout = html.Div([
    html.H1(
        "Процесс оказания образовательных услуг",
...
def update_dashboard(period):
    ...
    kpi_income = html.Div([
        html.H3("Доходы"), html.H1(f"{total_income:,} ₽")
    ], style={
        "background": "#2ECC71",
        "padding": "20px",
        "borderRadius": "10px",
    ...
    kpi_expense = html.Div([
        html.H3("Расходы"), html.H1(f"{total_expense:,} ₽")
    ], style={
        "background": "#E74C3C",
        "padding": "20px",
        "borderRadius": "10px",
    ...
    kpi_profit = html.Div([
        html.H3("Прибыль"), html.H1(f"{total_profit:,} ₽")
    ], style={
        "background": "#3498DB",
        "padding": "20px",
        "borderRadius": "10px",
    ...
    fig_line = px.line(
        df, x="Дата", y=["Доходы", "Расходы"],
        title="Динамика доходов и расходов"
    )

    fig_pie = px.pie(
        df, values="Расходы", names="Категория",
        title="Структура расходов по категориям"
    )

    # Здесь замена histogram на bar chart
    fig_hist = px.bar(
        df, x="Дата", y="Прибыль",
    ...
    fig_scatter = px.scatter(
        df, x="Студенты", y="Прибыль",
        title="Корреляция: Кол-во студентов vs Прибыль"
    )
